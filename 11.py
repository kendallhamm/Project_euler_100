# Hamm 10 June 2026
# very good problem to think about!
# padded with zeros so that when any of my 8 calcs left the box I would not return a bad answer. a bit clunky but works. 
data = """
...
matrix = [[int(x) for x in line.split()] for line in data.strip().splitlines()]

#print(matrix)
# matrix[0][2] is row 0 column 2, or value 22
# print(matrix[0][2])
...
for r in range(len(matrix)):
    for c in range(len(matrix[r])):
        try:
            north = m[r][c] * m[r-1][c] * m[r-2][c] * m[r-3][c]
        except IndexError:
            north = 0
        try: 
            northeast = m[r][c] * m[r-1][c+1] * m[r-2][c+2] * m[r-3][c+3]
        except IndexError:
            northeast = 0
        try:
            east = m[r][c] * m[r][c+1] * m[r][c+2] * m[r][c+3]
        except IndexError:
            east = 0
        try:
            southeast = m[r][c] * m[r+1][c+1] * m[r+2][c+2] * m[r+3][c+3]
        except IndexError:
            southeast = 0
        try:
            south = m[r][c] * m[r+1][c] * m[r+2][c] * m[r+3][c]
        except IndexError:
            south = 0
        try:
            southwest = m[r][c] * m[r+1][c-1] * m[r+2][c-2] * m[r+3][c-3]
        except IndexError:
            southwest = 0
        try:
            west = m[r][c] * m[r][c-1] * m[r][c-2] * m[r][c-3]
        except IndexError:
            west = 0
        try:
            northwest = m[r][c] * m[r-1][c-1] * m[r-2][c-2] * m[r-3][c-3]
        except IndexError:
            northwest = 0
        prods = [north , northeast , east , southeast , south , southwest , west , northwest]
        if max(prods) > largest_value:
            largest_value = max(prods)
